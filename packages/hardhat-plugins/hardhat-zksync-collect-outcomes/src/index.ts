...
extendConfig((config: HardhatConfig, userConfig: Readonly<HardhatUserConfig>) => {
    ...
    if (userCollects.deployments !== undefined) {
        ...
        config.paths.collects.deployments = {
            source: abspath(setting.source, config.paths.root),
            target: abspath(setting.target, config.paths.root),
            patterns: setting.patterns ?? [],
        }
    }
...
task(TASK_DEPLOY_ZKSYNC, 'deploy run only').setAction(async (taskArgs, { config }, runSuper) => {
    const deployOutput = await runSuper(taskArgs)
    if (config.paths.collects.deployments === undefined) {
        return deployOutput
    }
    const deploymentsRoot = config.paths.collects.deployments!.source
