...
const { VERBOSE, ZERO_ADDRESS } = require('../../../0.7/utils/constants');
    ...
    beforeEach(async function () {
        ...
        for (const endpoint of await generateEndpoints(chainIds)) {
            for (let i = 1; i <= numVersions; i++) {
                ...
                unwiredEndpoints[i].push(
                    await generateVersion(endpoint, chainIds, outboundProofType, i, false, v2, true)
                );
            }
        }
...
describe('MultiProofV2:', function () {
    ...
    beforeEach(async function () {
        ...
        dst.evmValidator = await deployNew('FPValidator', [ZERO_ADDRESS, ZERO_ADDRESS]); // both type 1 and 2 are the same impl, but not the same contract
