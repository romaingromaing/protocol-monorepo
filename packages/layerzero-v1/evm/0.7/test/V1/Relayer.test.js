...
describe('Relayer:', function () {
    let relayer, lzToken, ultraLightNode, deployer, user1, proxyOwner, badUser1, fakeContract;
    ...
    before(async function () {
        ({ badUser1, deployer, user1, proxyOwner, fakeContract, relayerDeployer, proxyAdminDeployer } = await getAddr(
            ethers
        ));
    });
    ...
    beforeEach(async function () {
        ...
        relayer = (await ethers.getContract('Relayer')).connect(relayerDeployer);
    ...
    it('upgrade() - only allow upgrade by admin', async function () {
        ...
        // approve for user1
        await relayer.setApprovedAddress(user1.address, true);
    ...
    it('withdrawQuotedFromULN() reverts with unapproved', async function () {
        await expect(relayer.connect(user1).withdrawQuotedFromULN(user1.address, 1)).to.be.revertedWith(
            'Relayer: not approved '
        );
    });

    it('withdrawQuotedFromULN() - reverts from non owner', async function () {
        await expect(relayer.connect(badUser1).withdrawQuotedFromULN(badUser1.address, 1)).to.revertedWith(
            'Relayer: not approved '
        );
    });
    ...
    it('getPrice() - reverts when paused', async function () {
        await relayer.setPause(true);
        const txParams = encodePackedParams(['uint16', 'uint'], [txType, extraGas]);
        await expect(
            relayer['getPrice(uint16,uint16,address,uint256,bytes)'](chainId, 1, user1.address, 10, txParams)
    ...
    it('getPrice() - reverts with invalid txParams', async function () {
        ...
        await expect(
            relayer['getPrice(uint16,uint16,address,uint256,bytes)'](chainId, 1, user1.address, 10, txParams)
        ).to.be.revertedWith('Relayer: wrong _adapterParameters size');
    });
    ...
    it('getPrice() - reverts with unsupported type', async function () {
        ...
        await expect(
            relayer['getPrice(uint16,uint16,address,uint256,bytes)'](1, 1, user1.address, 10, txParams)
        ).to.be.revertedWith('Relayer: unsupported txType');
    });

    it('getPrice() - reverts with bad tx types', async function () {
        const txParams = encodePackedParams(['uint16', 'uint'], [0, 1]);
        await expect(
            relayer['getPrice(uint16,uint16,address,uint256,bytes)'](1, 1, user1.address, 0, txParams)
        ).to.be.revertedWith('Relayer: unsupported txType');
    });
    ...
    it('getPrice() - reverts with not enough gas', async function () {
        ...
        await expect(
            relayer['getPrice(uint16,uint16,address,uint256,bytes)'](1, 1, user1.address, 10, txParams)
        ).to.be.revertedWith('Relayer: gas too low');
    });

    it('getPrice() - type 1 - returns 0 fees if transaction fees not set', async function () {
        const txParams = encodePackedParams(['uint16', 'uint'], [txType, extraGas]);
        expect(
            await relayer['getPrice(uint16,uint16,address,uint256,bytes)'](
                chainId,
                outboundProofType,
                user1.address,
                payloadLength,
    ...
    it("getPrice() - type 2 - reverts with 'dstNativeAmt too large'", async function () {
        ...
        await expect(
            relayer['getPrice(uint16,uint16,address,uint256,bytes)'](
                chainId,
                outboundProofType,
                user1.address,
                payloadLength,
    ...
    it('getPrice() - type 2 - returns correct price', async function () {
        ...
        const txParams = encodePackedParams(
            ['uint16', 'uint', 'uint', 'address'],
            [txType2, extraGas, dstNativeAmt, ZERO_ADDRESS]
        );
