...
const { deliverMsg, wireEndpoints, incrementCounterWithTest } = require('../util/helpers');
const { VARS, VERBOSE } = require('../../utils/constants');
    ...
    beforeEach(async function () {
        ...
        for (const endpoint of await generateEndpoints(chainIds)) {
            for (let i = 1; i <= numVersions; i++) {
                ...
                unwiredEndpoints[i].push(await generateVersion(endpoint, chainIds, outboundProofType, i));
            }
        }
...
describe('MultiProof:', function () {
    ...
    beforeEach(async function () {
        ...
        dst.evmValidator = await deployNew('MPTValidator'); // both type 1 and 2 are the same impl, but not the same contract
