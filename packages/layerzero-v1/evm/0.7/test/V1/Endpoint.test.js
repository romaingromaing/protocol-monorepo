...
describe('Endpoint:', function () {
    ...
    beforeEach(async function () {
        ...
        } = (await wireEndpoints([await generateVersion(endpoints[0], chainIds, outboundProofType, 1)]))[0]);
    });
...
describe('Endpoint Integration:', function () {
    ...
    beforeEach(async function () {
        ...
        unwiredEndpoints = await Promise.all(
            _endpoints.map(async (endpoint, index) => {
                return await generateVersion(endpoint, chainIds, outboundProofType, 1, true);
            })
        );
    ...
    it('retryPayload() - reverts when called multiple times or with wrong payloads', async function () {
        ...
        expect(
            await dst.lzEndpoint.hasStoredPayload(
                src.chainId,
        ...
        await expect(
            dst.lzEndpoint.retryPayload(src.chainId, src.counterMock.address, wrongPayload)
    ...
    it('receiveNonReentrant() - increment counter reverts when a bad relayer tries to send a msg twice', async function () {
        // testing for the
        const { payloadLength: payloadLengthA } = await dst.lzEndpoint.storedPayload(
            src.chainId,
            src.counterMock.address
        );
        ...
        const { payloadLength: payloadLengthB } = await dst.lzEndpoint.storedPayload(
            src.chainId,
            src.counterMock.address
        );
