...
const { deliverMsg, wireEndpoints, incrementCounterWithTestV2 } = require('../util/helpers');
const {
    generateEndpoints,
    generateVersion,
    callAsContract,
...
const { VERBOSE, ZERO_ADDRESS } = require('../../utils/constants');
...
describe.skip('MultiVersionV2:', function () {
    ...
    beforeEach(async function () {
        ...
        for (const endpoint of await generateEndpoints(chainIds)) {
            for (let i = 1; i <= numVersions; i++) {
                ...
                unwiredEndpoints[i].push(await generateVersion(endpoint, chainIds, outboundProofType, i, false, v2));
            }
        }
    ...
    it('incrementCounter() - works across multiple chains and versions', async function () {
        ...
        for (let version = 1; version <= numVersions; version++) {
            ...
            await wireEndpoints([ethereum, avax], true, v2);
            ...
            for (let i = 1; i <= numOfIncrements; i++) {
                ...
                for (let [src, dst] of [
                    ...
                    const tx = await src.counterMock.incrementCounter(dst.chainId, '0x', '0x', { value: 300000 });
                    ...
                    await checkOutboundNonceV2(src, dst, i);
                    await checkInboundNonceV2(dst, src, i);
...
describe('MultiProofV2:', function () {
    let chainIds = [1, 2];
    ...
    beforeEach(async function () {
        ...
        dst.evmValidator = await deployNew('MPTValidator01', [ZERO_ADDRESS, ZERO_ADDRESS]); // both type 1 and 2 are the same impl, but not the same contract
        ...
        await setOutboundProofType(src.ultraLightNode, src.lzEndpoint, src.counterMock, outboundProofType, dst.chainId);
    });
