...
describe('EndpointV2:', function () {
    ...
    beforeEach(async function () {
        ...
        ({
            ...
            await wireEndpoints(
                [await generateVersion(endpoints[0], chainIds, outboundProofType, 1, true, v2)],
                true,
                v2
            )
        )[0]);
    ...
    it('newVersion() - reverts when called by non owner', async function () {
        await expect(lzEndpoint.connect(user1).newVersion(user1.address)).to.be.revertedWith(
            'Ownable: caller is not the owner'
        );
    });

    it('newVersion() - reverts when called with 0x0', async function () {
        await expect(lzEndpoint.newVersion(ZERO_ADDRESS)).to.be.revertedWith(
            'LayerZero: new version cannot be zero address'
        );
    });
    ...
    it('newVersion() - reverts if max library size is hit', async function () {
        // initialize to 1
        expect(await lzEndpoint.latestVersion()).to.equal(1);
        let storageValue = await network.provider.send('eth_getStorageAt', [
            lzEndpoint.address,
        ...
        await network.provider.send('hardhat_setStorageAt', [
            lzEndpoint.address,
            '0x0',
            storageValue.replace('0001', 'FFFF'),
        ]);
        expect(await lzEndpoint.latestVersion()).to.equal(65535);

        await expect(lzEndpoint.newVersion(user1.address)).to.revertedWith(
            'LayerZero: can not add new messaging library'
        );
    });

    it('setDefaultSendVersion() - reverts when called by non owner', async function () {
        await expect(lzEndpoint.setDefaultSendVersion(0)).to.be.revertedWith(
            'LayerZero: default send version must > 0'
        );
    });

    it('setDefaultReceiveVersion() - reverts when called by non owner', async function () {
        await expect(lzEndpoint.setDefaultReceiveVersion(0)).to.be.revertedWith(
            'LayerZero: default receive version must > 0'
        );
    });

    it('setDefaultSendVersion() - reverts for 0', async function () {
        await expect(lzEndpoint.connect(user1).setDefaultSendVersion(0)).to.be.revertedWith(
            'Ownable: caller is not the owner'
        );
    });

    it('setDefaultSendVersion() - reverts when version is > latest Version', async function () {
        await expect(lzEndpoint.setDefaultSendVersion(123)).to.be.revertedWith(
            'LayerZero: invalid messaging library version'
        );
    });
    ...
    it('setDefaultReceiveVersion() - reverts when called by non owner', async function () {
        await expect(lzEndpoint.connect(user1).setDefaultReceiveVersion(1)).to.be.revertedWith(
            'Ownable: caller is not the owner'
        );
    });

    it('setDefaultReceiveVersion() - reverts when version is > latest Version', async function () {
        await expect(lzEndpoint.setDefaultReceiveVersion(123)).to.be.revertedWith(
            'LayerZero: invalid messaging library version'
        );
    });

    it('setDefaultReceiveVersion()', async function () {
        await expect(lzEndpoint.setDefaultReceiveVersion(1))
            .to.emit(lzEndpoint, 'DefaultReceiveVersionSet')
            .withArgs(1);
        expect(await lzEndpoint.defaultReceiveVersion()).to.equal(1);
    });
    ...
    it('send() - is blocked when on specific version, and txSend is set to max uint16', async function () {
        ...
        await expect(lzEndpoint.send(1, '0x', '0x', user1.address, user1.address, '0x')).to.be.revertedWith(
            'LayerZero: in BLOCK_VERSION'
        );
    });

    it('setConfig() - reverts during migration on default version', async function () {
        await lzEndpoint.setDefaultSendVersion(blockedVersion);
        await expect(lzEndpoint.setConfig(0, 0, 0, '0x')).to.be.revertedWith(
    ...
    it('setConfig() - reverts when new version number > latestVersion', async function () {
        await expect(lzEndpoint.setConfig(123, 0, 0, '0x')).to.be.revertedWith(
            'LayerZero: invalid messaging library version'
        );
    });

    it('setConfig() - reverts when version is BLOCK_VERSION', async function () {
        await expect(lzEndpoint.setConfig(blockedVersion, 0, 0, '0x')).to.be.revertedWith(
            'LayerZero: can not set config for BLOCK_VERSION'
        );
    });
    ...
    it('setConfig() - relayer set w/ default', async function () {
        ...
        expect(await lzEndpoint.getConfig(version, chainId, deployer.address, configType)).to.equal(
            relayerAddressBytes
        );
    });
    ...
    it('getConfig() - reverts during migration', async function () {
        ...
        await expect(lzEndpoint.getConfig(0, chainId, deployer.address, 1)).to.be.revertedWith(
            'LayerZero: no DEFAULT config while migration'
        );
    });

    it('getConfig() - reverts with invalid library version', async function () {
        await expect(lzEndpoint.getConfig(100, chainId, deployer.address, 1)).to.be.revertedWith(
            'LayerZero: invalid messaging library version'
        );
    });
    ...
    it('estimateFees() - reverts when type 2 doesnt contain dstNativeAddress', async function () {
        ...
        const adapterParamsType2 = encodePackedParams(
            adapterParams[2].types.slice(0, -1),
            adapterParams[2].values.slice(0, -1)
        );

        await expect(
            lzEndpoint.estimateFees(chainId, userApplication.address, payload, payInZro, adapterParamsType2)
        ).to.revertedWith('Relayer: wrong _adapterParameters size');
        await expect(
            ultraLightNode.estimateFees(chainId, userApplication.address, payload, payInZro, adapterParamsType2)
        ).to.revertedWith('Relayer: wrong _adapterParameters size');
    });
    ...
    it('setSendVersion()', async function () {
        ...
        await expect(lzEndpoint.setSendVersion(2))
            .to.emit(lzEndpoint, 'UaSendVersionSet')
            .withArgs(deployer.address, 2);
        ...
        await expect(lzEndpoint.setSendVersion(0))
            .to.emit(lzEndpoint, 'UaSendVersionSet')
    ...
    it('setReceiveVersion() - reverts when version is > latest Version', async function () {
        await expect(lzEndpoint.setReceiveVersion(123)).to.be.revertedWith(
            'LayerZero: invalid messaging library version'
        );
    });
    ...
    it('setReceiveVersion()', async function () {
        ...
        await expect(lzEndpoint.setReceiveVersion(2))
            .to.emit(lzEndpoint, 'UaReceiveVersionSet')
            .withArgs(deployer.address, 2);
        ...
        await expect(lzEndpoint.setReceiveVersion(0))
            .to.emit(lzEndpoint, 'UaReceiveVersionSet')
            .withArgs(deployer.address, 0);
    ...
    it('receivePayload() - reverts with invalid library', async function () {
        ...
        await expect(
            lzEndpoint.connect(badUser1).receivePayload(0, '0x', deployer.address, 1, 0, '0x')
        ).to.be.revertedWith('LayerZero: invalid library');
    });

    it('receivePayload() - reverts with invalid library w/ default', async function () {
        await expect(lzEndpoint.receivePayload(0, '0x', ZERO_ADDRESS, 1, 0, '0x')).to.be.revertedWith(
            'LayerZero: invalid default library'
        );
    });
    ...
    it('receivePayload() - reverts in blocked version', async function () {
        ...
        await expect(lzEndpoint.receivePayload(0, '0x', deployer.address, 1, 0, '0x')).to.be.revertedWith(
    ...
    it('getSendLibraryAddress() - reverts when non default send is BLOCK_VERSION', async function () {
        ...
        await expect(lzEndpoint.getSendLibraryAddress(deployer.address)).to.revertedWith(
            'LayerZero: send version is BLOCK_VERSION'
        );
    });
    ...
    it('getSendLibraryAddress() - reverts when default send is BLOCK_VERSION', async function () {
        ...
        await expect(lzEndpoint.getSendLibraryAddress(deployer.address)).to.revertedWith(
            'LayerZero: send version (default) is BLOCK_VERSION'
        );
    });
    ...
    it('getReceiveLibraryAddress() - reverts when non default receive is BLOCK_VERSION', async function () {
        ...
        await expect(lzEndpoint.getReceiveLibraryAddress(deployer.address)).to.revertedWith(
            'LayerZero: receive version is BLOCK_VERSION'
        );
    });
...
describe('Endpoint Integration V2:', function () {
    let chainIds = [1, 2];
    ...
    beforeEach(async function () {
        ...
        unwiredEndpoints = await Promise.all(
            _endpoints.map(async (endpoint, index) => {
                return await generateVersion(endpoint, chainIds, outboundProofType, 1, true, v2);
            })
        );
    ...
    it('NonceContract: increment()', async function () {
        const path = encodePackedParams(['address', 'address'], [dst.counterMock.address, src.counterMock.address]);
        const nonce = await src.nonceContract.outboundNonce(dst.chainId, path);
        await expect(
            callAsContract(src.nonceContract, src.ultraLightNode.address, 'increment(uint16,address,bytes)', [
                dst.chainId,
        ...
        expect(await src.nonceContract.outboundNonce(dst.chainId, path)).to.be.equal(nonce + 1);
    });

    it('NonceContract: increment() - reverts when sender is not valid send library', async function () {
        const path = encodePackedParams(['address', 'address'], [dst.counterMock.address, src.counterMock.address]);
        const nonce = await src.nonceContract.outboundNonce(dst.chainId, path);
        ...
        await expect(
            callAsContract(
                src.nonceContract,
                dst.ultraLightNode.address, // invalid uln address
                'increment(uint16,address,bytes)',
                [dst.chainId, src.counterMock.address, path]
            )
        ).to.be.revertedWith('NonceContract: msg.sender is not valid sendlibrary');

        // nonce doesnt update
        expect(await src.nonceContract.outboundNonce(dst.chainId, path)).to.be.equal(nonce);
    });
    ...
    it('send() - msg succeeds, even with reverted airdrop and relayer keeps amount', async function () {
        extraGas = 1001; // special blocked amount of gas
        const balanceBefore = await getBalance(dst.counterMock.address);
        const balanceBeforeRelayer = await getBalance(dst.relayer.address);

        const tx = await src.counterMock.incrementCounter(dst.chainId, '0x', '0x', { value: defaultMsgValue });
        // hits the arbitrary block of rejected ether for the counter mock contract, but tx still goes through
        await expect(deliverMsg(tx, src, dst, dst.counterMock.address, { value: extraGas })).to.not.be.reverted;

        expect((await getBalance(dst.counterMock.address)).sub(balanceBefore)).to.equal(0); // does not get rejected and the gas
        expect((await getBalance(dst.relayer.address)).sub(balanceBefore)).to.equal(extraGas); // relayer keeps the extra gas
    });

    it("validateTransactionProof() - reverts with 'source counter is not trusted'", async function () {
        const tx = await src.counterMock.incrementCounter(dst.chainId, '0x', '0x', { value: defaultMsgValue });

        // reset trusted remote
        await dst.counterMock.setTrustedRemote(src.chainId, '0x');

        const reason = encodeParams(['string'], ['source counter is not trusted']);
        const path = encodePackedParams(['address', 'address'], [src.counterMock.address, dst.counterMock.address]);
        await expect(deliverMsg(tx, src, dst, dst.counterMock.address, { value: extraGas }))
            .to.emit(dst.lzEndpoint, 'PayloadStored')
            .withArgs(src.chainId, path, dst.counterMock.address, 1, '0x', '0x08c379a0'.concat(reason.slice(2)));
    });
    ...
    it('Send() - reverts if refund reverts', async function () {
        await expect(
            callAsContract(
                src.counterMock,
                src.counterMock.address,
                'incrementCounter(uint16,bytes,bytes)',
                [dst.chainId, '0x', '0x'],
                defaultMsgValue
            )
    ...
    it('retryPayload() - reverts when called multiple times or with wrong payloads', async function () {
        ...
        const payload = decodedLogs[0].events[4].value ?? '0x';
        await checkCounters(src, dst, 0, 0); // doesnt update because not enough gas

        // stored payload key is no longer srcAddress, its bytes array packed of srcAddress and dstAddress
        const storedPayloadKey = encodePackedParams(
            ['address', 'address'],
            [src.counterMock.address, dst.counterMock.address]
        );

        // check if any stored payload
        expect(await dst.lzEndpoint.hasStoredPayload(src.chainId, storedPayloadKey)).to.equal(true);

        // trying with invalid payload, fail()
        let wrongPayload = payload + '01';
        await expect(dst.lzEndpoint.retryPayload(src.chainId, storedPayloadKey, wrongPayload)).to.be.revertedWith(
            'LayerZero: invalid payload'
        );

        // trying with valida payload, success()
        await dst.lzEndpoint.retryPayload(src.chainId, storedPayloadKey, payload);
        await checkCounters(src, dst, 0, 1);

        // using the same transaction proof from the relayer should result in a revert
        await expect(dst.lzEndpoint.retryPayload(src.chainId, storedPayloadKey, payload)).to.be.revertedWith(
            'LayerZero: no stored payload'
        );

        // check if any stored payload
        expect(await dst.lzEndpoint.hasStoredPayload(src.chainId, storedPayloadKey)).to.equal(false);
    });
    ...
    it("retryPayload() - emits event and nonce isn't updated until enough gas is sent", async function () {
        ...
        await checkOutboundNonceV2(src, dst, 1);
        await checkInboundNonceV2(dst, src, 1);

        // try again with too little gas which should fail
        try {
            await dst.lzEndpoint.retryPayload(src.chainId, storedPayloadKey, payload, { gasLimit: 22628 });
        } catch (e) {
        ...
        // finally enough gas
        await expect(dst.lzEndpoint.retryPayload(src.chainId, storedPayloadKey, payload, { gasLimit: 1000000 }))
            .to.emit(dst.lzEndpoint, 'PayloadCleared')
            .withArgs(parseInt(src.chainId), storedPayloadKey.toLowerCase(), 1, dst.counterMock.address);
    ...
    it('incrementCounter() - reverts when sending delivering msg twice', async function () {
        ...
        // using the same transaction proof from the relayer should result in a revert
        await expect(deliverMsg(tx, src, dst, dst.counterMock.address, { confirmations: 16 })).to.be.revertedWith(
    ...
    it('sendNonReentrant() - increment counter reverts when a bad oracle tries to send a msg twice', async function () {
        ...
        await expect(
            src.counterMock.incrementCounter(dst.chainId, '0x', '0x', { value: defaultMsgValue })
        ).to.be.revertedWith('LayerZero: no send reentrancy');
    });
    ...
    it('receiveNonReentrant() - increment counter reverts when a bad relayer tries to send a msg twice', async function () {
        ...
        // testing for the
        const { payloadLength: payloadLengthA } = await dst.lzEndpoint.storedPayload(src.chainId, storedPayloadKey);
        ...
        const { payloadLength: payloadLengthB } = await dst.lzEndpoint.storedPayload(src.chainId, storedPayloadKey);
        expect(payloadLengthB).to.equal(2); // length of the 0x6666 is 2
    });
    ...
    it('incrementCounter() - multiple chains and nonces update accordingly', async function () {
        await incrementCounterWithTestV2(src, dst);
        await incrementCounterWithTestV2(src, dst);

        await incrementCounterWithTestSameChainV2(src);
        await incrementCounterWithTestSameChainV2(dst);
        await incrementCounterWithTestV2(src, dst);
        await incrementCounterWithTestV2(src, dst);
        ...
        await incrementCounterWithTestV2(dst, src);
    });
    ...
    it('forceResumeReceive() - nonces are updating properly', async function () {
        ...
        await expect(deliverMsg(tx, src, dst, dst.counterMock.address)).to.revertedWith(
            'LayerZero: in message blocking'
        );

        // nonce should remain the same at the destination
        await checkCounters(src, dst, 0, 0);
        await checkOutboundNonceV2(src, dst, 2);
        await checkInboundNonceV2(dst, src, 1);
        ...
        await expect(
            callAsContract(dst.lzEndpoint, user1.address, 'forceResumeReceive(uint16,bytes)', [
                src.chainId,
                encodePackedParams(['address'], [storedPayloadKey]),
            ])
        ...
        await expect(
            ...
            .withArgs(parseInt(src.chainId), storedPayloadKey.toLowerCase());
        ...
        await checkCounters(src, dst, 0, 1);
        await checkOutboundNonceV2(src, dst, 2);
        await checkInboundNonceV2(dst, src, 2);
        ...
        expect(
            await dst.lzEndpoint.hasStoredPayload(src.chainId, encodePackedParams(['address'], [storedPayloadKey]))
        ).to.equal(false);
    ...
    it.skip('withdraw() - via treasury and relayer withdraw', async function () {
        ...
        await incrementCounterWithTestV2(src, dst, { value: 3000000000 });
        ...
        await src.relayer.connect(user1).withdrawFee(user2.address, withdrawAmount);
        ...
        await src.relayer.withdrawFee(user2.address, withdrawAmount);
